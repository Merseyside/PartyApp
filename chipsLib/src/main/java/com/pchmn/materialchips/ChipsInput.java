...
public class ChipsInput extends ScrollViewMaxHeight {
    ...
    private void init(AttributeSet attrs) {
        ...
        // adapter
        mChipsAdapter = new ChipsAdapter(mContext, this, mRecyclerView, mIsSelectable);
    ...
    private OnChipClickListener onClickListener = chip -> {
        if (mIsSelectable) {
            chip.setChipSelected();

            if (selectedlistener != null) {
                selectedlistener.onSelected(chip.isChipSelected(), chip.getChip());
            }
        }
    };
    ...
    public List<? extends ChipInterface> getAllChips() {
    ...
    public boolean isShowChipDetailed() {
        return mShowChipDetailed && !mIsSelectable;
    }
    ...
    public List<ChipInterface> getSelectedChips() {
        List<ChipInterface> selectedChips = new ArrayList<>();

        for (int i = 0; i < mRecyclerView.getChildCount(); i++) {
            if (mRecyclerView.getChildAt(i) instanceof ChipView) {
                ChipView chip = (ChipView) mRecyclerView.getChildAt(i);

                if (chip.isChipSelected()) {
                    selectedChips.add(chip.getChip());
                }
            }
        }
    ...
    public ChipView getChipViewById(Object id) {

        for (int i = 0; i < mRecyclerView.getChildCount(); i++) {
            if (mRecyclerView.getChildAt(i) instanceof ChipView) {

                ChipView chip = (ChipView) mRecyclerView.getChildAt(i);
                if (chip.getChip().getId() == id) return chip;
            }
        }
