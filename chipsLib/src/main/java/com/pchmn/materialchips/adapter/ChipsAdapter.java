...
public class ChipsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public ChipsAdapter(Context context, ChipsInput chipsInput, RecyclerView recycler, Boolean isSelectable) {
        mContext = context;
    ...
    private void handleClickOnEditText(ChipView chipView, final int position) {
        ...
        if(mChipsInput.isShowChipDetailed()) {
            chipView.setOnChipClicked(chip -> {
                // get chip position
                int[] coord = new int[2];
                chip.getLocationInWindow(coord);
